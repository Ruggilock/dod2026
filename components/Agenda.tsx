    ...
    <section id="agenda" className="mx-auto max-w-6xl px-4 py-24">
      ...
      <div className="mb-8 flex gap-4">
        <button
          ...
          className={`rounded px-6 py-3 font-mono text-xs uppercase tracking-widest transition-all ${
            activeDay === "day1"
              ? "bg-black text-white"
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex-1">
                <div className="mb-2 flex items-center gap-3">
                  <span className="font-mono text-sm text-black/60">{item.time}</span>
                  <span className={`rounded px-2 py-1 font-mono text-xs uppercase ${typeColors[item.type]}`}>
                    {item.type}
                  </span>
                </div>
                <h3 className="text-lg font-medium text-black">{item.title}</h3>
                {item.speaker && (
                  <p className="mt-1 text-sm text-black/60">por {item.speaker}</p>
                )}
              </div>
            </div>
          </div>
